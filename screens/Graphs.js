...
const Graphs = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://192.168.0.101:5000/data`, {
        params: {
  ...
  return (
    <ScrollView style={{ padding: 20 }}>
      ...
      <Picker selectedValue={topic} onValueChange={setTopic}>
        ...
        <Picker.Item label="Light Intensity" value="luxVal" />
        <Picker.Item label="Bulb Current" value="current2" />
        <Picker.Item label="Peltier Current" value="current1" />
        <Picker.Item label="Fan Current" value="current3" />
      </Picker>
